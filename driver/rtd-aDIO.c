...
//----------------------------------------------------------------------------
    ...
    $Id: rtd-aDIO.c 134034 2022-06-22 12:52:27Z kkelly $
*/
...
#include <asm/irq.h>
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 11, 0)
#define WAIT_Q_T wait_queue_entry_t
#else
#define WAIT_Q_T wait_queue_t
#endif
...
static int debug = 0; // 0x7f; // 0x7f for all debug flags
...
static const int irq_decode[NUM_IRQ_ELEM] = { 0, 5, 7, 10, 11, 12,
	...
	defined(__INTEL_KABY_LAKE_CHIPSET) || \
...
static const unsigned int addr_decode[NUM_ADDR_ELEM] = {
	...
	0x9C0,
#elif defined(__AMD_GSERIES_CHIPSET) || defined (__INTEL_CHIEF_RIVER_CHIPSET) || defined(__INTEL_KABY_LAKE_CHIPSET) || defined (__INTEL_BAYTRAIL_CHIPSET)
	0xEC0,
#else
	0x450,
#endif
...
static void adio_unregister_device(board_t * dev)
{
	...
	if (dev && (dev->flags & INITIALIZED)) {
		...
		/**
		 * 	Need to enable again if using a shared IRQ line
		 */
...
static int adio_register_device(board_t * dev)
{
	...
	/**
	 *	request_irq() enables irq similar to if enable_irq() was called
...
#endif
#if defined(__INTEL_KABY_LAKE_CHIPSET)
	printk(KERN_INFO "INTEL KABY LAKE CHIPSET was detected\n");
#endif
